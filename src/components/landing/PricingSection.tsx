    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className={`inline-flex items-center justify-center w-12 h-12 rounded-xl mb-6 ${
          ...
          {name === 'Free' && <Sparkles className={popular ? 'text-white' : 'text-white'} size={24} />}
          {name === 'Professional' && <Zap className="text-white" size={24} />}
          {name === 'Business' && <Crown className={popular ? 'text-white' : 'text-white'} size={24} />}
        </div>
        ...
        {/* CTA Button */}
        <button className={`w-full py-4 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center space-x-2 mb-8 ${
          popular
          ...
          {features.map((feature, idx) => (
            <div key={idx} className="flex items-start space-x-3">
              <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 ${
                ...
                <Check className={popular ? 'text-white' : 'text-blue-600'} size={14} strokeWidth={3} />
              </div>
              <span className={`text-sm leading-relaxed ${
                popular ? 'text-blue-50' : 'text-gray-700'
              }`}>
                {feature}
              </span>
            </div>
